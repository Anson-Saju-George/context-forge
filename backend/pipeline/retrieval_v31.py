...
def term_signature(result: dict) -> set[str]:
  features = chunk_features(result)
  return set(features["concrete_hits"]) | set(features["operational_hits"])


def diverse_by_terms(results: list[dict], top_k: int, per_document: int = 4) -> list[dict]:
  grouped = {}
  order = []
  for result in results:
    document_id = result.get("document_id", "")
  ...
  for document_id in order:
    ...
    for result in grouped[document_id]:
      ...
      if chunk_id in selected_ids:
        continue
      signature = term_signature(result)
      new_terms = signature - seen_terms
      if picked > 0 and signature and not new_terms:
        continue
      document_selected.append(result)
      selected_ids.add(chunk_id)
      seen_terms |= signature
      picked += 1
      if picked >= per_document:
        break
    selected_by_document.append(document_selected)

  selected = []
  for index in range(per_document):
    for document_selected in selected_by_document:
      if index >= len(document_selected):
        continue
      selected.append(document_selected[index])
      if len(selected) >= top_k:
        return selected
...
def retrieve_context(query: str, chunks: list[dict], requested_top_k: int | None = None) -> tuple[list[dict], dict]:
  ...
  if intent == "extraction" and len(document_hints) > 1:
    max_top_k = settings()["config"].get("limits", {}).get("max_top_k", 12)
    target_k = max(target_k, min(len(document_hints) * 4, max_top_k))
  ...
  for document_id in document_hints:
    ...
    routed_candidates.extend(bm25_retrieve(query, document_chunks, min(24, len(document_chunks))))
    if intent == "extraction":
      routed_candidates.extend(technical_density_candidates(document_chunks, min(32, len(document_chunks))))
  ...
  if intent == "extraction" and len(document_hints) > 1:
    selected = diverse_by_terms(reranked_candidates, target_k, per_document=4)
  else:
